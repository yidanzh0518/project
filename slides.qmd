...
-   The goal is to develop a package that implements the GloVe algorithm to generate word embedding matrices from EHR data.
...
-   The second step is to use the co-occurrence matrix to create the pointwise mutual information (PMI) matrix, which quantifies the association between a word and a context word. SPPMI, another widely used method in machine learning, is also implemented through a function. Both of them is build on another function that obtain the joint and singleton frequency of the co-occurrence counts.
...
The aim for the package is to adapt the methods we learned in this class to optimize the functions and reduce the computational overhead.
