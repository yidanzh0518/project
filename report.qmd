...
```
    ...
    summary_table <- data[, .(npatient = uniqueN(SUBJECT_ID),ncode = uniqueN(ICD9_CODE))]

    ncount_summary <- data[, .(n_rows = .N), by = SUBJECT_ID][
      , .(
    ...
    # Create a table with row names and format it with kable
...
print(cooccurrence_matrix_static)
print(cooccurrence_matrix_dynamic)
```
...
#| echo: false
#| include: false
data <- fread(file="~/project/data/DIAGNOSES_ICD.csv")
